...
def p_control_loop(robot, target_positions, start_positions, kp=0.5, control_freq=50):
    """
    P control loop using terminal-based keyboard input.
        ...
        while True:
            try:
                ...
                while key is not None:
                    ...
                    if key == 'x':
                        print("\nExit command detected, returning to start position...")
                    ...
                    if key in joint_controls:
                        ...
                        if joint_name in target_positions:
                            ...
                            print(f"\rUpdated {joint_name}: {current_target} -> {new_target}    ", end="", flush=True)
                ...
                # Extract current joint positions
                current_positions = {}
                for obs_key, value in current_obs.items():
                    if obs_key.endswith('.pos'):
                        motor_name = obs_key.removesuffix('.pos')
                        calibrated_value = apply_joint_calibration(motor_name, value)
                        current_positions[motor_name] = calibrated_value
...
def main():
    ...
    try:
        ...
        p_control_loop(robot, target_positions, start_positions, kp=0.5, control_freq=50)
